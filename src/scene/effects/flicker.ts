...
export type Flicker = Disposable & {
  /**
   * Registers one light that stutters as a unit.
   *
   * Everything in `lights` goes dark together, which is the point of taking a
   * group rather than a mesh: a headlamp and the cone it throws are two meshes
   * and one light, and so are a street lamp's bulb and the pool below it. A cone
   * of light left lying on the road under a bulb that has just gone out is
   * exactly what would give the trick away.
   */
 ...
 */
export function createFlicker(clock: Clock, rules: FlickerRules): Flicker {
  const faults: Fault[] = [];
  let elapsed = 0;
  ...
  const stop = clock.each((dt) => {
    ...
    for (const fault of faults) {
      if (elapsed >= fault.until) {
        fault.stuttering = !fault.stuttering;
        fault.until = elapsed + between(fault.stuttering ? rules.stutter : rules.steady);
      }
      ...
      const lit = !fault.stuttering || Math.floor(elapsed * rules.rate) % 2 === 0;
